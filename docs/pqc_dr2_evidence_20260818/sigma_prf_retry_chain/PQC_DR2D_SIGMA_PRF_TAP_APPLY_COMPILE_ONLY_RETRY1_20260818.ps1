...
$evidence = ".\PQC_DR2D_sigma_prf_tap_apply_compile_only_retry1_evidence_20260818.txt"
...
try {
  & {
    ...
    "RETRY_EVIDENCE=$([IO.Path]::GetFullPath($evidence))"
    ...
    if ($originalEvidenceText -notmatch "missing required keyword-only argument.*d_slots") {

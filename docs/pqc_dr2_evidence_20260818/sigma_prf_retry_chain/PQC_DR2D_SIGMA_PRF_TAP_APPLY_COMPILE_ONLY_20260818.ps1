...
$evidence = ".\PQC_DR2D_sigma_prf_tap_apply_compile_only_evidence_20260818.txt"
...
$expectedPatchHash = "BCBFEC3F904FB4B64E6457320BD7750ADF2BE66CB760F2C8EBC2E68AF85F5204"
...
$newDiagnosticFiles = @(
  ".\phoenix_sdr_dsp\pqc\kernels\dr2d_mlkem512_kpke_sigma_prf_tap.cc",
...
foreach ($path in $newDiagnosticFiles) {
  if (Test-Path -LiteralPath $path) {
    throw "REFUSED: additive diagnostic path already exists: $path"
  }
}
...
try {
  & {
    "===== DR2D SIGMA-PLUS-PRF TAP: NON-DISPATCH PROVENANCE ====="
    ...
    "PATCH_APPLIED=True"
    & git diff --no-ext-diff -- $newDiagnosticFiles

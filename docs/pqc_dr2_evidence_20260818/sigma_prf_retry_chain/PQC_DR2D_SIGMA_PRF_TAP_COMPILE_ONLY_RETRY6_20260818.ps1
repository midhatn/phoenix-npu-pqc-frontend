...
$evidence = ".\PQC_DR2D_sigma_prf_tap_apply_compile_only_retry6_evidence_20260818.txt"
$retry5Evidence = ".\PQC_DR2D_sigma_prf_tap_apply_compile_only_retry5_evidence_20260818.txt"
$retry4Evidence = ".\PQC_DR2D_sigma_prf_tap_apply_compile_only_retry4_evidence_20260818.txt"
...
try {
  & {
    "===== DR2D SIGMA-PLUS-PRF TAP RETRY6: NON-DISPATCH PROVENANCE ====="
    "UTC_START=$([DateTime]::UtcNow.ToString('o'))"
    "REPO_ROOT=$repoRoot"
    "PATCH_APPLY_REQUIRED=False"
    ...
    "RETRY3_EVIDENCE=$([IO.Path]::GetFullPath($retry3Evidence))"
    "RETRY4_EVIDENCE=$([IO.Path]::GetFullPath($retry4Evidence))"
    "RETRY5_EVIDENCE=$([IO.Path]::GetFullPath($retry5Evidence))"
    "RETRY6_EVIDENCE=$([IO.Path]::GetFullPath($evidence))"
    "ORIGINAL_EVIDENCE=$([IO.Path]::GetFullPath($originalEvidence))"
    "RETRY1_EVIDENCE=$([IO.Path]::GetFullPath($retry1Evidence))"
    "RETRY2_EVIDENCE=$([IO.Path]::GetFullPath($retry2Evidence))"
    ...
    "===== RETRY4 STRUCTURAL EVIDENCE GATE ====="
    if (-not (Test-Path -LiteralPath $retry4Evidence -PathType Leaf)) { throw "REFUSED: retry4 evidence is missing: $retry4Evidence" }
    $retry4EvidenceText = Get-Content -LiteralPath $retry4Evidence -Raw
    foreach ($required in @("DR2D SIGMA-PLUS-PRF TAP RETRY4: NON-DISPATCH PROVENANCE", "CORRECTIVE_PATCH_APPLIED=True", "HOST_AND_NO_DISPATCH_MLIR_CONTRACTS=PASS", "REFUSED: outer-factory correction is incomplete: missing return _specialized_program()", "COMPILE_EXIT=1", "NPU_DISPATCH_ATTEMPTED=False")) { if (-not $retry4EvidenceText.Contains($required)) { throw "REFUSED: retry4 evidence lacks required marker: $required" } }
    if ($retry4EvidenceText -match "COMPILE_ONLY_RETRY4_GATE=PASS" -or $retry4EvidenceText -match "NPU_DISPATCH_ATTEMPTED=True") { throw "REFUSED: retry4 is not expected pre-dispatch compile-helper failure" }
    "RETRY4_STRUCTURAL_COMPILE_HELPER_FAILURE=CONFIRMED"
    "RETRY4_EVIDENCE_PRESERVED=True"

    "===== RETRY5 STRUCTURAL EVIDENCE GATE ====="
    if (-not (Test-Path -LiteralPath $retry5Evidence -PathType Leaf)) { throw "REFUSED: retry5 evidence is missing: $retry5Evidence" }
    $retry5EvidenceText = Get-Content -LiteralPath $retry5Evidence -Raw
    foreach ($required in @("DR2D SIGMA-PLUS-PRF TAP RETRY5: NON-DISPATCH PROVENANCE", "RETRY4_STRUCTURAL_COMPILE_HELPER_FAILURE=CONFIRMED", "===== PROTECTED PRODUCTION PRE-HASHES =====", "===== ACCEPTED V2 DIAGNOSTIC PRE-HASHES =====", "===== CURRENT APPLIED DIAGNOSTIC PRE-HASHES =====", "NPU_DISPATCH_ATTEMPTED=False")) { if (-not $retry5EvidenceText.Contains($required)) { throw "REFUSED: retry5 evidence lacks required marker: $required" } }
    if ($retry5EvidenceText -match "COMPILE_ONLY_BEGIN=True" -or $retry5EvidenceText -match "COMPILE_ONLY_RETRY5_GATE=PASS" -or $retry5EvidenceText -match "NPU_DISPATCH_ATTEMPTED=True") { throw "REFUSED: retry5 is not expected pre-compile no-dispatch failure" }
    "RETRY5_STRUCTURAL_PRECOMPILE_FAILURE=CONFIRMED"
    "RETRY5_EVIDENCE_PRESERVED=True"
...
normalized_factory_lines = [line.strip() for line in factory_source.splitlines()]
if "return _specialized_program()" in normalized_factory_lines:
    raise SystemExit("REFUSED: outer factory calls the CallableDesign instead of returning it")
if "return _specialized_program" not in normalized_factory_lines:
    raise SystemExit("REFUSED: outer factory does not return the CallableDesign exactly")
print(

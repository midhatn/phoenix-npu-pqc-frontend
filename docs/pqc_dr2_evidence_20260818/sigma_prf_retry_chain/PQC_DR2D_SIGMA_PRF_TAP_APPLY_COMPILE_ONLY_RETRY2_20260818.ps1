...
$evidence = ".\PQC_DR2D_sigma_prf_tap_apply_compile_only_retry2_evidence_20260818.txt"
...
$expectedPatchHash = "C1E09EBFD8B5A854547BB4B1BE83C75A3C6BA2E59AB1C562AB4DB541F08700DD"
...
$currentlyAppliedDiagnosticExpected = [ordered]@{
  ...
  ".\phoenix_sdr_dsp\pqc\dr2d_mlkem512_kpke_sigma_prf_tap_graph.py" = "C324ECE810D5FCBFE3363BD7DB0EAAE3F8515154B5FE08A41B12BE7E79D6F6FB"
...
try {
  & {
    ...
    "RETRY2_EVIDENCE=$([IO.Path]::GetFullPath($evidence))"
    ...
    if ($retry1EvidenceText -notmatch "REFUSED: original evidence does not contain the reviewed missing-d_slots failure") {
      throw "REFUSED: retry1 evidence is not the reviewed pre-apply wrapper refusal"
    }
    if ($retry1EvidenceText -notmatch "NPU_DISPATCH_ATTEMPTED=False") {
      throw "REFUSED: retry1 evidence does not prove no dispatch"
    }
    if ($retry1EvidenceText -match "CORRECTIVE_PATCH_APPLIED=True" -or
        $retry1EvidenceText -match "COMPILE_ONLY_RETRY1_GATE=PASS" -or
        $retry1EvidenceText -match "NPU_DISPATCH_ATTEMPTED=True") {
      throw "REFUSED: retry1 evidence is not the expected pre-apply, non-dispatch refusal"
    }
    "RETRY1_WRAPPER_REFUSAL=CONFIRMED"

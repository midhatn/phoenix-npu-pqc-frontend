...
- Regression pin: commit [`3ca0193cea9e2c39ec670a65f93e1dd43c969f22`](https://github.com/Xilinx/mlir-aie/commit/3ca0193cea9e2c39ec670a65f93e1dd43c969f22), dated 2026-08-14, message "Retain executable per kernel handle to fix run_chain use-after-free" ([PR #3545](https://github.com/Xilinx/mlir-aie/pull/3545)). This commit is v1.4.1 plus 13 commits and additionally contains the `run_chain` executable-lifetime fix required by the parallel-DMA milestones.
...
- `Runtime()` used as a `with`-block is removed at v1.4.1. The current form is `Runtime(seq_fn, fn_args=[...])`, verified against [`python/iron/runtime/runtime.py`](https://github.com/Xilinx/mlir-aie/blob/3ca0193/python/iron/runtime/runtime.py) at the pin.
...
All iron-based milestones in `tests/` (M3, M5–M15, M15b, M17, M17p) target the v1.4.1 API. Do not check out an mlir-aie release earlier than v1.4.1 without also reverting the tests to the older API.
